...
export function bootstrapPlanningProcessor(instance: DbInstance): void {
  const { db } = instance;
  const existing = db.select().from(processorRegistry).where(eq(processorRegistry.processor_id, 'planning')).get();
  if (existing) return;

  const now = new Date().toISOString();
  db.insert(processorRegistry).values({
    processor_id: 'planning',
    name: 'Planning Processor',
    base_url: `${API_BASE_URL}/api/kanban-processor/planning`,
    health_endpoint: '/health',
    hooks: ['on-enter', 'on-update', 'on-action', 'can-exit', 'on-exit'],
    sla_seconds: 300,
    max_sla_seconds: 86400,
...
export function bootstrapCommitProcessor(instance: DbInstance): void {
  const { db } = instance;
  const existing = db.select().from(processorRegistry).where(eq(processorRegistry.processor_id, 'commit')).get();
  if (existing) return;
  ...
  db.insert(processorRegistry).values({
    ...
    base_url: `${API_BASE_URL}/api/kanban-processor/commit`,
    health_endpoint: '/health',
    hooks: ['on-enter', 'on-update', 'on-action', 'can-exit', 'on-exit'],
    sla_seconds: 300,
    max_sla_seconds: 86400,
...
export function bootstrapExploreProcessor(instance: DbInstance): void {
  const { db } = instance;
  const existing = db.select().from(processorRegistry).where(eq(processorRegistry.processor_id, 'explore')).get();
  if (existing) return;

  const now = new Date().toISOString();
  db.insert(processorRegistry).values({
    processor_id: 'explore',
    name: 'Explore Processor',
    base_url: `${API_BASE_URL}/api/kanban-processor/explore`,
    health_endpoint: '/health',
    hooks: ['on-enter', 'on-update', 'on-action', 'can-exit', 'on-exit'],
    sla_seconds: 300,
    max_sla_seconds: 86400,
